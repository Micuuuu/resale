import React, { useState } from "react";
import { useSelector } from "react-redux/es/exports";
import { selectAllProducts } from "../../store/categories/category.selector";
import { selectUserDataMapById } from "../../store/user-data/user-data.selector";
import { selectCurrentUser } from "../../store/user/user.selector";
import ProductCard from "../../components/product-card/product-card.component";
import "./for-you.styles.scss";

const ForYou = () => {
  const [searchedValue, setSarchedValue] = useState("");
  const currentUserData = useSelector(selectCurrentUser);
  const products = useSelector(selectAllProducts);
  const userData = useSelector(selectUserDataMapById);

  return (
    <div className="for-you">
      {currentUserData && products && userData ? (
        <>
          <div className="shop-search-container">
            <h1>From your followers</h1>
            <div className="search-input">
              <svg version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 487.95 487.95">
                <g>
                  <g>
                    <path
                      fill="#b6b6b6"
                      d="M481.8,453l-140-140.1c27.6-33.1,44.2-75.4,44.2-121.6C386,85.9,299.5,0.2,193.1,0.2S0,86,0,191.4s86.5,191.1,192.9,191.1    c45.2,0,86.8-15.5,119.8-41.4l140.5,140.5c8.2,8.2,20.4,8.2,28.6,0C490,473.4,490,461.2,481.8,453z M41,191.4    c0-82.8,68.2-150.1,151.9-150.1s151.9,67.3,151.9,150.1s-68.2,150.1-151.9,150.1S41,274.1,41,191.4z"
                    />
                  </g>
                </g>
              </svg>
              <input onChange={(e) => setSarchedValue(e.target.value)} placeholder="Search" type="text" />
            </div>
          </div>
          <div className="for-you-products">
            {userData[currentUserData.uid].following.map((follow) => {
              return products.map((prod) =>
                prod.owner.uid === follow && prod.name.toLowerCase().includes(searchedValue.toLowerCase()) ? (
                  <ProductCard key={prod.id} products={prod} title={"All"} currentUser={currentUserData} />
                ) : (
                  ""
                )
              );
            })}
          </div>
        </>
      ) : (
        ""
      )}
    </div>
  );
};

export default ForYou;
